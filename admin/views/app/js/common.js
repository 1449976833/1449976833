...
function addattach_img(fileurl,imgsrc,aid, width, height, alt){
	if (editorMap['editor_content'].designMode === false){
		alert('请先切换到所见所得模式');
	}else if (imgsrc != "") {
		editorMap['editor_content'].insertHtml('<a target=\"_blank\" href=\"'+fileurl+'\" id=\"ematt:'+aid+'\"><img src=\"'+imgsrc+'\" title="点击查看原图" alt=\"'+alt+'\" border=\"0\" width="'+width+'" height="'+height+'"/></a>');
	}
}
function addattach_file(fileurl,filename,aid){
	if (editorMap['editor_content'].designMode === false){
		...
		editorMap['editor_content'].insertHtml('<span class=\"attachment\"><a target=\"_blank\" href=\"'+fileurl+'\" >'+filename+'</a></span>');
	}
}
...
function autosave(act){
    ...
    if (act == 3 || act == 4){
        ...
        var allow_remark = $("#allow_remark").attr("checked") == 'checked' ? 'y' : 'n';
        var is_blank = $("#is_blank").attr("checked") == 'checked' ? 'y' : 'n';
      ...
      if (title==""){
        ...
        var lansort = $.trim($("#lansort").val());
        var palt = $.trim($("#palt").val());
        var downurl1 = $.trim($("#downurl1").val());
        var downurl2 = $.trim($("#downurl2").val());
		...
		var content = $('#editor_content').val();
		var excerpt = $('#editor_excerpt').val();
        var tag = $.trim($("#tag").val());
        var top = $("#top").attr("checked") == 'checked' ? 'y' : 'n';
        var sortop = $("#sortop").attr("checked") == 'checked' ? 'y' : 'n';
        var allow_remark = $("#allow_remark").attr("checked") == 'checked' ? 'y' : 'n';
        var allow_tb = $("#allow_tb").attr("checked") == 'checked' ? 'y' : 'n';
